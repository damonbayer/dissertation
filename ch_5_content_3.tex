...
\begin{figure}
    ...
    \caption[CRPS summaries for hospital occupancy forecasts.]{CRPS summaries for hospital occupancy forecasts at 1, 2, and 4-week horizons for three simulated data sets. Lower CRPS is better.}
    \label{ch_5:fig:simulated_crps_comparison_boxplot_data_hospitalizations_plot}
\end{figure}

We also specifically assess our each model's ability to estimate the timing and size of the peak hospital occupancy.
...
In general, the model where \( 1 / \kappa(t) \) is informed by the genetic data tends to have the smallest credible interval widths and is overconfident about peak timing and value early on, but can accurately forecast these quantities when more data is included in the model.
...
\begin{figure}
    \centering
    \includegraphics[width=1.0\columnwidth]{figures/ch_5/simulated_peak_assessment_value_plot}
...
Summaries of scores for these forecasts are depicted in Figure~\ref{ch_5:fig:simulated_peak_crps_boxplot_plot}, while the score at each forecast time is shown in Figure~\ref{ch_5:fig:simulated_peak_crps_plot}.
...
\begin{figure}
    ...
    \caption[CRPS summaries for peak hospital occupancy.]{CRPS summaries for peak hospital occupancy timing and size for three simulated data sets. Lower CRPS is better.}
...
This wave lasted from roughly December 2021 to March 2022, but we use data beginning in May 2021 to forecast this time period, as this time period includes the previous wave of cases.
...
This is a clear departure from the models we fit in our simulation study, where the second peak's size compared to the first peak, so we chose to fit the same models as in the simulation study, but modified also include modeling the case detection rate with a Gaussian Markov random field.
...
It would be even harder to anticipate this changing relationship in a real forecasting scenario with a novel variant.
We simply chose to incorporate some additional flexibility into the model in a place that might be useful.
