...
Forecasting is particular challenging when conditions in the environment change.
These changes could be due to a variety of factors including policy decisions, vaccination campaigns, treatment developments, or phenotypic changes in the disease due to evolution.
These phenotypic changes may result in increased transmissibility, severity, or immune evasion, leading to surges in cases, hospitalizations, or deaths.
...
In this work, we focus on developing forecasting methodology for these times when a new variant becomes dominant.

Forecasting in changing environments has previously been addressed by including time-varying parameters in forecast models.
These time-varying parameters can be treated as fixed inputs or estimated via parametric or non-parametric methods.
Many models focus on incorporating a time-varying transmission rate.
For example, \citet{Zhou2020Semiparametric} modeled the time-varying transmission rate via a Gaussian process.
In \citet{Iyaniwura2022Mathematical}, the authors use survey data to derive separate time-varying contact rates for different subpopulations and incorporate these derived rates into their transmission rate model.
...
In \citet{ODea2021semi} time-varying transmission rate, reporting probability, and observation variance are modeled with random walks.
Similarly, \citet{Gibson2020real} also models time-varying transmission rate and case detection rate with random walks.
Additionally, \citet{morozova2021one} incorporates time-varying contact rate, transmission rate, recovery rate, severe fraction, hospital discharge rate, and hospital case fatality ratio into a Bayesian model.
While none of the above models allow for the average immune duration to vary in time, their methods could easily be adapted to model this dynamic behavior.

Despite the apparent relevance of the novel variants, a recent comprehensive review of 136 models noted that not one of the papers analyzed incorporated variant prevalence data \citep{Nixon2022evaluation}.
Since that review's publication, we are aware of two papers which have attempted to use this data \citep{Du2023Incorporating, Rashed2022Covid}.
...
We take inspiration from \citet{Figgins2021variant}, which models variant proportions over time using a Dirichlet-multinomial likelihood to estimate variant-specific effective reproduction numbers.
Analogously, we assume that the observed count of sequences of a novel variant has a beta-binomial distribution whose mean is a product of the total number of observed genetic sequences and the proportion of the infectious population with the novel variant.
...
We demonstrate competitive or superior ability to forecast cases, hospital occupancy, ICU occupancy, and deaths, as judged by continuous rank probability scores.
...
\end{equation}
where \( \beta \) and \( N \) are defined as in \eqref{ch_5:eqn:transition_rates}, and \( \epsilon \in [0, 1] \) is a factor that represents partial immunity to the novel variant conferred by a recent infection from  another variant.
After some time, all the population in the \( O \) subscript compartments will have transitioned to the \( N \) subscript compartments and the model again behaves like the one in Figure~\ref{ch_5:fig:model_diagram}.
...
The second is a slight variation on the first, where we make the immune duration, rather than the basic reproductive number, vary over time.
...
For the medium data, the model with \( 1 / \kappa(t) \) informed by the genetic data again fits the data well up to the four-week forecast horizon.
...
To numerically assess the quality of these forecasts, we compute the continuous rank probability score (CRPS) for each forecast at 1, 2, and 4-week horizons.
Boxplots summarizing the scores of the hospitalization forecasts are displayed in Figure~\ref{ch_5:fig:simulated_crps_comparison_boxplot_data_hospitalizations_plot}.
Analogous plots for forecasts of cases, ICU occupancy, and deaths are presented in Figures~\ref{ch_5:fig:simulated_crps_comparison_boxplot_data_new_cases_plot}--\ref{ch_5:fig:simulated_crps_comparison_boxplot_data_new_deaths_plot}.
...
We observe that across all simulated scenarios and data streams, the model where \( 1 / \kappa(t) \) is informed by the genetic data achieves the lowest (best) scores.
For the slow takeover data, the model where \( 1 / \kappa(t) \) is modeled by a GMRF performs better than the model where \( R_0(t) \) is a GMRF.
For the medium takeover data, the two GMRF models are competitive with each other, while the \( R_0(t) \) GMRF model outperforms the \( 1 / \kappa(t) \) GMRF model for the fast takeover data.

\begin{figure}
    \centering
    \includegraphics[width=1.0\columnwidth]{simulated_crps_comparison_boxplot_data_hospitalizations_plot}
    ...
    \label{ch_5:fig:simulated_crps_comparison_boxplot_data_hospitalizations_plot}
\end{figure}

We also specifically assess each model's ability to estimate the timing and size of the peak hospital occupancy.
Posterior credible intervals for the peak timing and peak value are presented in Figures~\ref{ch_5:fig:simulated_peak_assessment_time_plot} and \ref{ch_5:fig:simulated_peak_assessment_value_plot}.
In general, the model where \( 1 / \kappa(t) \) is informed by the genetic data tends to have the smallest credible interval widths and is overconfident about peak timing and value early on, but can accurately forecast these quantities when later data is included in the model.
...
\begin{figure}
    ...
    \caption[Posterior credible intervals for peak hospital occupancy timing for simulated data sets.]{Posterior credible intervals for the time at which hospital occupancy reaches its maximum in three simulated data sets.
...
\begin{figure}
    ...
    \caption[Posterior credible intervals for peak hospital occupancy for simulated data sets.]{Posterior credible intervals for the maximum hospital occupancy in three simulated data sets.
...
Summaries of scores for these forecasts are depicted in Figure~\ref{ch_5:fig:simulated_peak_crps_boxplot_plot}.
...
When the models are fit to the medium and fast takeover data, the model where \( R_0(t) \) is a GMRF performs the best.

\begin{figure}
    \centering
    \includegraphics[width=1.0\columnwidth]{simulated_peak_crps_boxplot_plot}
    ...
    \label{ch_5:fig:simulated_peak_crps_boxplot_plot}
\end{figure}
...
The BA.1 sequences are the ``novel" variant, and the non-BA.1 sequences are the ``other" variants.
...
This is a clear departure from the models we fit in our simulation study, where the second peak's size compared to the first peak is the same for all data streams.
Because of this, we chose to fit the same models as in the simulation study, but modified them to also include modeling the case detection rate with a Gaussian Markov random field.
...
These observations are reflected in Figure~\ref{ch_5:fig:real_data_crps_comparison_boxplot_data_hospitalizations_plot}, where we observe that the models where \( 1 / \kappa(t) \) varies in time achieve lower CRPS values at for the two and four-week forecast horizons, compared to the model where \( 1 / \kappa(t) \) is constant.
Analogous figures for the other data streams are presented in Figures~\ref{ch_5:fig:real_data_crps_comparison_boxplot_data_new_cases_plot}--\ref{ch_5:fig:real_data_crps_comparison_boxplot_data_new_deaths_plot}.
...
In Figures~\ref{ch_5:fig:real_data_peak_assessment_time_plot} and \ref{ch_5:fig:real_data_peak_assessment_value_plot}, we present posterior credible intervals for the peak timing and peak value for hospital occupancy.
...
This is demonstrated in Figure~\ref{ch_5:fig:real_data_peak_crps_boxplot_plot}, which shows a summary of the CRPS for forecasts of peak hospital occupancy time and amount.
...
\begin{figure}
    \centering
    \includegraphics[width=1.0\columnwidth]{real_data_crps_comparison_boxplot_data_hospitalizations_plot}
    ...
    \label{ch_5:fig:real_data_crps_comparison_boxplot_data_hospitalizations_plot}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=1.0\columnwidth]{real_data_peak_assessment_time_plot}
    \caption[Posterior credible intervals for peak hospital occupancy timing for real data sets.]{Posterior credible intervals for the time at which hospital occupancy reaches its maximum in Orange County and California data sets.
    Horizontal and vertical dashed lines indicate the true peak hospitalization time.}
...
\begin{figure}
    ...
    \caption[Posterior credible intervals for peak hospital occupancy for real data sets.]{Posterior credible intervals for the maximum hospital occupancy in Orange County and California  data sets.
    Horizontal dashed lines indicate the true peak hospital occupancy, while the vertical dashed lines indicate the true peak hospital occupancy time.}
...
\begin{figure}
    \centering
    \includegraphics[width=1.0\columnwidth]{real_data_peak_crps_boxplot_plot}
    ...
    \label{ch_5:fig:real_data_peak_crps_boxplot_plot}
\end{figure}

\section{Discussion}
\label{ch_5:sec:discussion}

We developed a Bayesian model that integrates time series of cases, hospitalizations, ICU admissions, and deaths, as well as genetic data to create forecasts during time periods where a novel virus variant becomes dominant.
Our approach is based on the idea that the introduction of a novel variant leads to increased infections due primarily to immune escape.
...
When the new variant becomes dominant quickly (6 weeks), this model is competitive with the standard model, which performs the best.
...
In contrast, the case and death counts do directly influence estimates of latent incidence, so they would likely need to be modeled in a real-world use case.
...
Certain lineages may also be able to be identified more easily than others (e.g. the  B.1.1.7 variant was easily identified by S-gene target failures in PCR testing).
Some effort could be made to correct for this in either the model or the data collection.
Finally, it should be possible to use the estimated proportion of the infectious population with the novel variant to inform model parameters which might differ between variants (e.g. the infection hospitalization ratio or duration of infectiousness).
However, understanding which of these parameters may differ between variants may be difficult to do in real-time.
