...
Throughout the COVID-19 pandemic, the need for these forecasts has been addressed by a large number of researchers who have developed and applied new models to predict disease burden.
As the pandemic progressed, the effects of novel variants such as the Alpha, Delta, and Omicron variants were repeatedly accompanied by waves of increased cases, hospitalizations, and deaths.
...
These two works use a deep learning framework and develop long short-term memory (LSTM) models which incorporate many data sources, including pre-processed estimates of variant proportions.
...
While compartmental models for dynamics of pathogens with multiple strains have been used to study disease transmission for a long time (see \citet{Kucharski2016Capturing} for an overview) these models have not been incorporated into forecasting efforts.
Rather than adapting one of these models, which necessitate many additional compartments and parameters compared to the single strain models, we propose a modification to a single strain model.
Our proposed method models the average duration of immunity as a flexible function of the proportion of the infectious population with a novel variant.
...
We assume that the observed count of sequences of the novel variant has a beta-binomial distribution whose mean is a product of the total number of observed genetic sequences and this proportion.
...
\begin{itemize}
    \item Modeling hospitalization with/for is somewhat complicated.
    \item Assume that with is dominated by for time we care about.
    \item Hospitalization may change for other reasons, but we assume negligible unless there is a natural disaster or something.
    \item We assume that genetic sequences from each variant happens at the same rate, probably not true.
    \item I think there is a note in GISAID or on outbreak.info about this.
    \item Assume variants don't differ any way, but you can accommodate
    ...
    \item variants can emerge without leading to  waves
\end{itemize}
