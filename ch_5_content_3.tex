...
These changes could be due to a variety of factors including non-pharmaceutical interventions, vaccination campaigns, treatment developments, or phenotypic changes in the disease due to evolution.
...
While none of the above models allow for the average immune duration to vary in time, a topic of interest in this chapter, their methods could easily be adapted to model this dynamic behavior.
...
We demonstrate competitive or superior ability to forecast cases, hospital occupancy, ICU occupancy, and deaths, as judged by the continuous rank probability score - a popular proper scoring rule for probabilistic forecasts \citep{gneiting2007strictly}.
...
We seek to integrate the following data sources into our modeling: time series of daily numbers of cases, infected hospital occupants, infected ICU occupants, deaths due to the infection, genetic sequences from a novel variant, and genetic sequences from other variants.
...
\( \exp \left\{ \alpha_0 + 4^{\alpha_2} \ln \left( \alpha_1^* \right) \left( 1/4  \right)^{\alpha_2} \right\} = \alpha_1^* \exp \alpha_0 \).
Recalling that \( \exp \alpha_0 \) is the maximum immune duration, we see that  \( \alpha_1^* \in [0,1] \) is simply a scaling factor, which determines the minimum duration, based on the maximum duration.
...
\end{equation}
This approach is borrowed from population genetics literature which commonly assumes this logistic growth of the novel variant proportion \citep{Lacerda2014Population, Zhao2023mechanism}.
...
Adding a GMRF component requires one additional parameter per observation time.
...
That is, people who are susceptible to all variant (\( S_A \)) types and those who are only recovered from other (\( R_O \)) variants can become infected with the novel variant (\( (E_N \)).
...
We manipulate the size of this wave and the speed that the novel variant takes over by altering \( \beta \) and \( \epsilon \) in the different simulation settings.
...
For each dataset, we used four Markov chains run in parallel to draw a total of 1000 posterior samples.
