...
In our simulation study and application in Section~\ref{ch_5:sec:results}, we observe the non-genetic data at a weekly resolution, while the genetic data is reported at a daily resolution.
...
In some cases, we can allow certain parameters to vary in time, to capture changing dynamics without needing to add more compartments to our model.
...
Because the total population size is fixed at size \( N \), and we take \( H_0 \) , \( U_0 \), and \( D_0 \) to match the reported values of the hospitalizations, ICU occupancy, and deaths in the data at \( t_0 \), we are left with \( E_0 \), \( I_0 \), and \( R_0 \) as free parameters.
...
\end{equation}
respectively, where \( \delta \left( t_l \right) \) is the proportion of currently infectious individuals who are infected with the novel variant, and \( \rho^V \) is the overall case sequencing rate (relative to observed cases.
...
In some models, we allow \( \rho^W \) to vary over time, in which case we replace \( \rho^W \) with \( \rho^W \left( t_l \right) \).
...
We could instead define the time-varying parameters as functions of \( \delta(t) \), the proportion of currently infectious individuals who are infected with the novel variant.
In principle, we can do this for any of these parameters (or any other parameters in the model that might differ by variant), but we only consider this framing for \( \kappa \) in our simulation study and application.
We model \( \kappa \) as dependent on \( \delta(t) \) by
\begin{equation}
...
\end{equation}
where \( \alpha_2 > 0 \).
This ensures that \( 1 / \hat{\kappa}(0) = 1 / \hat{\kappa}(1) = \exp \left( \alpha_0 \right)\) and that \(  \kappa(t) \) is maximized (or \( 1 / \kappa(t) \) is minimized) when \( \delta = 0.5 \), while being relatively flexible.
...
\end{equation}
where \( t^* \) is some time point for which \( \delta(t^*) = \frac{ \exp \left\{ \iota_0 \right\} }{\exp \left\{ \iota_0 \right\} + 1} \).
Then \( \iota_2 = \frac{\ln \left( \frac{0.99}{1 - 0.99} \right) - \ln \left( \frac{0.01}{1 - 0.01} \right)}{\iota_2^*} \), where \( \iota_2^* \) is the amount of time the novel variant takes to grow from 1\% of the infectious population to 99\% of the infectious population.

I SHOULD VERIFY THESE INTERPRETATIONS WITH SOME SAMPLE CALCULATIONS OR FIGURES.
...
We assume that all univariate non-GMRF distributed parameters are \textit{a priori} independent and list our prior assumptions in Table~tk.
