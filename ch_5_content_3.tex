...
While these models forecast several data streams, we present only those related to hospitalization in the main text, as this is the most relevant for consumers of these forecasts.
...
For the slow data, the forecasts where \( 1 / \kappa(t) \) is informed by the genetic data closely match the data up to the four-week ahead forecasts.
...
Boxplots summarizing the scores of the hospitalization forecasts are displayed in Figure~\ref{ch_5:fig:simulated_crps_comparison_dotplot_data_hospitalizations_plot}.
...
Summaries of scores for these forecasts are depicted in Figure~\ref{ch_5:fig:simulated_peak_crps_dotplot_plot}.
...
The plots show weekly counts of cases, hospital and ICU occupancy with COVID-19, reported deaths due to COVID-19, as well as counts of virus sequences for BA.1 and all lineages, and the proportion of BA.1 lineages.
The gray highlighted regions indicate the times for which we produce forecasts.}
...
This is demonstrated in Figure~\ref{ch_5:fig:real_data_peak_crps_dotplot_plot}, which shows a summary of the CRPS for forecasts of peak hospital occupancy time and amount.
